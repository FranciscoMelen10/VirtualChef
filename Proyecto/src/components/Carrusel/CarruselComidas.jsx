  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        axios
          .get(
            'https://virtualchef.pockethost.io/api/collections/horario/records',
          )
          .then((response) => {
            // Conseguir el id del horario
            setTiempo(
              response.data.items.filter((item) => item.nombre === horario),
            );

            // Filtrar las recetas por el id del horario
            setRecetas(
              datos.filter((datos) => datos.horarioId === tiempo[0].id),
            );
          });
      } catch (error) {
        console.log(error);
